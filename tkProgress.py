...
class TkProgress(tk.Toplevel):
    ...
    def DB_to_df(self,tasks_res,Profile, Details = False):
        if Details:
                ...
                for p in Properties:
                    if p in D_temp:
                        if D_temp[p]['type'] == 'number':
                            ...
                            content = start
                        elif D_temp[p]['type'] == 'checkbox':
                            content = bool(D_temp[p]['checkbox'])
                        else:
                            #print(D_temp[p]['type'])
                            content = None 
...
#Update Progress 
    ...
    def Notion_Sync(self,share_link,Profile,callback,Misc = False,Details = False):
...
###Program:
        ...
        if True:
            self.count = 0
            df = self.DB_to_df(res,Profile,Details)
            try:
