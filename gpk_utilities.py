...
def Plan_sep_to_dict(plan_dict):
    TEMP =  {'Family':[],'Health':[],'Personal Development':[],'Career':[]}
...
def SEC_Stat(Plan,sec = 'Time', agg = True, 
         sections = {1:'Health',2:'Family',3:'Personal Development',4:'Career'}):
    "Return a Dict of days that Reflects the Time Distribution by Sections"
    _null = {ORI:0 for ORI in sections.values()}
    if agg:
        TEMP = copy.copy(_null)
    else:
        TEMP = {Day: copy.copy(_null) for Day in Plan}
#     print(TEMP)
    for day in Plan:
...
def Plan_Legit(Load):
    def KR_Check(KR):
        ERROR = []
        KR_Dict = KR[1]
        #Checking for deadline format, 2021/11/07
        if KR_Dict['deadline'] is not None:
            try:
                DATE(KR_Dict['deadline'])
            except:
                ERROR.append( ('deadline',KR_Dict['deadline']))
    ...
    for Cat in ['Priority_Task','Special_Task','Recursive_Task']:
        ...
        try:
            ...
            for task in CAT:
                if task.ID not in _IDs:
                    _IDs.append(task.ID)
                else:
                    _Count += 1
                    print('{_Count}.ERROR FOUND:')
                    print(f'\t->ID {task.ID} is not unique')
    ...
    instruction = """
        ...
        1. All time must be numbers 
        2. All difficulty must be numbers 
        3. Standard Format: {time:xx,difficulty:xx,deadline:yyyy-mm-dd} is followed after each KR
        4. All Goal ID must be unique within the same category
                      """
    #Checking if ID are unique, 2021/12/13
...
def weekday_today(timezone = None):
...
#         weeks,freq = GAP_Filler(dates,freq)#Fill the potential Gaps
    ...
    def Plot_Sec(self, n = None , time_frame = 'Day',sec = 'Time', 
                 ...
                 dim = 111,Group = 'date_done', plt_method = None,
                 kargs = {}):
        """
    ...
    def add(self,task_name,task_ID,task_time,task_diff,task_des,ddl = None, RETURN = False):
        ...
        try:
            if task_ID  in list(self.todos['ID']):
                print(f"ERROR,ID {task_ID} Already Exsit")
                tk.messagebox.showerror(title = 'ID not unique error', message = f'ID {task_ID} Already Exsit')
                return 
...
if __name__ == '__main__':
    ...
    # #'https://www.notion.so/511539815e434eafad2fF5f329ed55b574?v=24f7815ecbd249819744d7fbcd6c9828'
    # User_name = 'Leo_TEST'##
